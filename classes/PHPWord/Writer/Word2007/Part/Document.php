...
class Document extends AbstractPart
{
    ...
    public function getObjectAsText($element){
        ...
        if($element instanceof PHPWord_Section_Text) {
            echo "Instance Of: Text";
            $this->_writeText($objWriter, $element);
        } elseif($element instanceof PHPWord_Section_TextRun) {
            echo "Instance Of: TextRun";
            $this->_writeTextRun($objWriter, $element);
        } elseif($element instanceof PHPWord_Section_Link) {
            echo "Instance Of: TextLink";
            $this->_writeLink($objWriter, $element);
        } elseif($element instanceof PHPWord_Section_Title) {
            echo "Instance Of: Title";
            $this->_writeTitle($objWriter, $element);
        } elseif($element instanceof PHPWord_Section_TextBreak) {
            echo "Instance Of: TextBreak";
            $this->_writeTextBreak($objWriter);
        } elseif($element instanceof PHPWord_Section_PageBreak) {
            echo "Instance Of: PageBreak";
            $this->_writePageBreak($objWriter);
        } elseif($element instanceof PHPWord_Section_Table) {
            echo "Instance Of: Table";
            $this->_writeTable($objWriter, $element);
        } elseif($element instanceof PHPWord_Section_ListItem) {
            echo "Instance Of: ListItem";
            $this->_writeListItem($objWriter, $element);
        } elseif($element instanceof PHPWord_Section_Image ||
                 $element instanceof PHPWord_Section_MemoryImage) {
            echo "Instance Of: Image/Memory";
            $this->_writeImage($objWriter, $element);
        } elseif($element instanceof PHPWord_Section_Object) {
            echo "Instance Of: Object";
            $this->_writeObject($objWriter, $element);
        } elseif($element instanceof PHPWord_TOC) {
            echo "Instance Of: TOC";
            $this->_writeTOC($objWriter);
        } elseif($element instanceof Table) {
            $this->_writeTable($objWriter, $element);
        }
        else echo "THIS DOES NOT HAVE A KNOWN INSTANCE TYPE!";
        return trim(preg_replace("/[\x1-\x8\xB-\xC\xE-\x1F-\t+]/", "", $objWriter->getData()));

    }
    public function getTableObjectAsText($element){
