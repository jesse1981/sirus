...
function htmltodocx_paths() {
    ...
    function _htmltodocx_get_style($element, $state) {
      ...
      $inheritable_props = $this->htmltodocx_inheritable_props();
    ...
    function htmltodocx_insert_html(&$phpword_element, $html_dom_array, &$state = array()) {
      ...
      // we go:
      $this->htmltodocx_insert_html_recursive($phpword_element, $html_dom_array, $state);
    }
    ...
    function htmltodocx_insert_html_recursive(&$phpword_element, $html_dom_array, &$state = array()) {
      ...
      $allowed_children = $this->htmltodocx_html_allowed_children($state['parents'][0]);
      ...
      foreach ($html_dom_array as $element) {
        ...
        $state['current_style'] = $this->_htmltodocx_get_style($element, $state);
        ...
        switch ($element->tag) {
          ...
          case 'table':
            if (in_array('table', $allowed_children)) {
              ...
              $this->htmltodocx_insert_html_recursive($phpword_element, $element->nodes, $state);
              array_shift($state['parents']);
          ...
          case 'tr':
            if (in_array('tr', $allowed_children)) {
              ...
              $state['textrun'] = $phpword_element->createTextRun();
              $state['textrun']->addText($this->htmltodocx_clean_text($element->innertext),  $state['current_style']);
            }     
          break;
          ...
          case 'th':
            if (in_array($element->tag, $allowed_children) && $state['table_allowed']) {
              ...
              array_unshift($state['parents'], $element->tag);
              $this->htmltodocx_insert_html_recursive($state['table_cell'], $element->nodes, $state);
              array_shift($state['parents']); 
            }
            else {
          ...
          case 'ul':
            ...
            $this->_htmltodocx_add_list_start_end_spacing_style($state);
          ...
          // elements (these will be within a text run).
            if (!isset($state['textrun'])) {
              $text = $this->htmltodocx_clean_text(trim($element->innertext));
            }
            else {
              $text = $this->htmltodocx_clean_text($element->innertext);
            }
            if (!empty($text)) {
              if (!isset($state['textrun'])) {
                $state['textrun'] = $phpword_element->createTextRun();
          ...
          case 'img':
            ...
            elseif (strpos($element_src, '/') === 0) {
              $src = $this->htmltodocx_doc_root() . $element_src;
            }
            else {
              $src = $this->htmltodocx_doc_root() . $state['base_path'] . $element_src; 
            }
